...
/*
 쓰레드 생성을 위해서 사용
 아규먼트인 thread는 쓰레드가 성공적으로 생성되었을때 생성된 쓰레드를 식별하기 위해서 사용되는 쓰레드 식별자이다
 아규먼트인 attr 은 쓰레드 특성을 지정하기 위해서 사용하며, 기본 쓰레드 특성을 이용하고자 할경우에 NULL 을 사용한다
 아규먼트인 start_routine는 분기시켜서 실행할 쓰레드 함수
 아규먼는인 arg는 쓰레드 함수의 인자이다
*/

// 1.2 pthread_join
int pthread_join(pthread_t th, void** thread_return);
/*
 아규먼트 th는 기다릴(join)할 쓰레드 식별자
 아규먼트 thread_return은 쓰레드의 리턴(return) 값이다
...
/*
 detach 는 "떼어내다" 라는 뜻을 가지며 main 쓰레드에서 pthread_create 를 이용해 생성된 쓰레드를 분리시킨다
 함수는 식별번호th인 쓰레드를 detach 시키는데,
 detach 되었을경우 해당(detach 된) 쓰레드가 종료될경우 pthread_join 을 호출하지 않더라도 즉시 모든 자원이 해제(free) 된다
*/

// 1.4 pthread_exit
void pthread_exit(void* retval);
/*
 thread_exit 는 현재 실행중인 쓰레드를 종료시키고자 할때 사용한다
...
/*
 ...
 routine이 쓰레드가 종료될때 호출되는 함수이다
 cleanup handlers 는 주로 자원을 되돌려주거나, mutex 잠금등의 해제를 위한 용도로 사용된다
 mutex 영역에서 pthread_exit 가 호출되어 버릴경우 다른쓰레드에서 영원히 block 될수 있기 때문
...
/*
 pthread_cleanup_push(5번) 와 함께 사용되며, install 된 cleanup handler 을 제거하기 위해서 사용된다
...
// 2. 쓰레드 동기화 함수 쓰레드 동기화와 관련된 함수들이다.
...
/*
 ...
 보호하고자 하는 데이타를 다루는 코드영역을 단지 한번에 하나의 쓰레드만 실행가능 하도록 하는 방법으로
 공유되는 데이타를 보호한다
 ...
 본 mutex 특성을 이용하기 원한다면 NULL 을 사용하면 된다
...
/*
 ...
 만약 이미 다른 쓰레드에서 mutex lock 를 얻어서 사용하고 있다면
 다른 쓰레드에서 mutex lock(뮤텍스 잠금) 을 해제할때까지(사용할수 있을때까지) 블럭 된다
 만약 다른 어떤 쓰레드에서도 mutex lock 을 사용하고 있지 않다면, 
 즉시 mutex lock 을 얻을수 있게 되고 critcal section 에 진입하게 된다
 critcal section 에서의 모든 작업을 마쳐서 사용하고 있는 mutex lock 이 더이상 필요 없다면 
 pthread_mutex_unlock 를 호출해서 mtuex lock 를 되돌려준다
*/
...
/*
 조건변수 cond에 시그날을 보낸다. 
 시그날을 보낼경우 cond에서 기다리는(wait) 쓰레드가 있다면 쓰레드를 깨우게 된다(봉쇄가 풀림)
 만약 조건변수 cond를 기다리는 쓰레드가 없다면, 아무런 일도 일어나지 않게되며,
 여러개의 쓰레드가 기다리고 있다면 그중 하나의 쓰레드에게만 전달된다. 
...
/*
 조건변수 cond에서 기다리는(wait) 모든 쓰레드에게 신호를 보내서, 
 깨운다는 점을 제외하고는 pthread_cond_signal과 동일하게 작동한다.
*/
...
/*
 ...
 기본은 PTHREAD_CREATE_JOINABLE 이며, pthread_detach를 이용해서 생성된 쓰레드를 detach 상태로 만들었을경우 또는
 pthread_attr_setdetachstate함수를 이용해서 쓰레드를 detache 상태로 변경시켰을경우 PTHREAD_CREATE_DETACHED 상태가 된다.
*/
...
/*
 쓰레드에서 시그널은 서로 공유된다. 
 그런이유로 만약 프로세스에 시그널이 전달되면 프로세스가 생성된 모든 쓰레드로 시그널이 전달된다.
